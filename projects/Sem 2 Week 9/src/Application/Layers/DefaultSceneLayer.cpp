...
#pragma endregion 
		...
		//}
		// Orthographic Version of shadow Caster Camera
		GameObject::Sptr shadowCasterOrtho = scene->CreateGameObject("Orthographic Shadow Light");
		{
			// Set position in the scene
			shadowCasterOrtho->SetPostion(glm::vec3(3.0f, 3.0f, 5.0f));
			shadowCasterOrtho->LookAt(glm::vec3(0.0f));
		
			// Create and attach a renderer for the monkey
			ShadowCamera::Sptr shadowCam = shadowCasterOrtho->Add<ShadowCamera>();
			shadowCam->SetProjection(glm::ortho(-10.0f, 10.0f, -10.0f, 10.0f, 0.1f, 75.0f));
		}
